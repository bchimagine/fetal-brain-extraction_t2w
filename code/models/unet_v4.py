...
def decoder_block_(inputs, conv_output, n_filters=64, kernel_size=3, strides=3, dropout=0.3):
...
def unet_v4(input_shape, cls_num=2):
    ...
    init_ = conv2d_block(init, n_filters=32)
    ...
    outputs = tf.keras.layers.Conv2D(cls_num, (1, 1), activation='softmax', name='seg')(outputs)

    flat_2 = tf.keras.layers.Flatten(name='flat_2')(encoder_output)
    flat_2 = tf.keras.layers.Dropout(0.3)(flat_2)
    flat_3 = tf.keras.layers.Dense(1024, activation='sigmoid')(flat_2)
    flat_3 = tf.keras.layers.Dropout(0.3)(flat_3)
    qual = tf.keras.layers.Dense(1, activation='sigmoid', name='qual')(flat_3)


    return tf.keras.Model(inputs=inputs, outputs=[outputs, qual])

if __name__ == '__main__':
    from keras_flops import get_flops

    model = unet_v4(input_shape=(256, 256, 1), cls_num=2)
    model.summary()
