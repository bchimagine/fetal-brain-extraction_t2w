...
def IcNet(input_shape, cls_num=2):
    inp = Input(shape=input_shape)
    x = Lambda(lambda x: x / 1.0)(inp)

    # (1/2)
    y = Lambda(lambda x: tf.image.resize(x, size=(int(x.shape[1]) // 2, int(x.shape[2]) // 2)),
               name='data_sub2')(x)
    ...
    # (1/4)
    y_ = Lambda(lambda x: tf.image.resize(x, size=(int(x.shape[1]) // 2, int(x.shape[2]) // 2)),
                name='conv3_1_sub4')(z)
    ...
    out = Conv2D(cls_num, 1, activation='sigmoid', name='conv6_cls')(y)
